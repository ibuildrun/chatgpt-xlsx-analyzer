  ...
  switch (name) {
    case 'confirmAction':
      return (
        <div className="my-2 p-3 border border-yellow-500 bg-yellow-50">
          ...
          <div className="text-sm mb-3">{args.message || 'Confirm this action?'}</div>
          {isResolved ? (
      ...
      return (
        <div className="my-2 p-3 border border-purple-500 bg-purple-50">
          ...
          <div className="text-sm font-mono">
            {args.sheetName}!{args.range}
          </div>
          {args.message && (
            <div className="text-xs text-gray-600 mt-1">{args.message}</div>
          )}
          <button
            onClick={() => onOpenSpreadsheet?.(args.sheetName, args.range)}
    ...
    case 'getRange':
      if (toolResult?.result?.cells) {
        const cells = toolResult.result.cells as Array<Array<{ value: unknown }>>;
        return (
          <div className="my-2 p-3 border border-gray-300 bg-white">
            <div className="text-[10px] uppercase tracking-wider text-gray-500 mb-2">
              TABLE DATA: {args.sheetName}!{args.range}
            </div>
      ...
      return (
        <div className="my-2 p-2 border border-gray-300 bg-gray-50 text-xs">
          <span className="font-mono">getRange({args.sheetName}, {args.range})</span>
    ...
    case 'updateCell':
      return (
        <div className="my-2 p-3 border border-orange-500 bg-orange-50">
          ...
          <div className="text-sm font-mono">
            {args.sheetName}!{args.cell} = {JSON.stringify(args.value)}
          </div>
          {isResolved && (
